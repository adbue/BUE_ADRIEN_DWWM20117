...
--
-- Hôte : 127.0.0.1:3308
-- Généré le : lun. 05 avr. 2021 à 18:57
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `CA_Supplier` (IN `p_sup_nom` VARCHAR(40), IN `p_date` INT)  BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `facture` (IN `p_id_order` INT)  BEGIN
    ...
    SELECT ord_id AS 'N° Facture', ord_order_date AS 'Date de commande', ord_reception_date AS 'Date de livraison',
          ...
          ((ode_quantity * ode_unit_price)- ode_discount) AS 'TOTAL HT'
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Lst_Rush_Orders` (IN `p_rush` VARCHAR(30))  BEGIN
	SELECT *
    FROM orders
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Lst_Suppliers` ()  BEGIN
...
(8, 27, '249.99', 'barb002', NULL, 2, 5, 'Noir', 'Athos', 'Cu\'rabitur pellentesque posuere luctus. Sed et risus vel quam pharetra pretium non quis odio. Praesent varius risus vel orci ultrices tincidunt.', 0, 4, '2016-06-14 00:00:00', NULL, 'jpg'),
...
CREATE TRIGGER `after_products_update` AFTER UPDATE ON `products` FOR EACH ROW BEGIN
        ...
        IF NEW.pro_stock_alert< NEW.pro_stock AND NEW.pro_id = 8  THEN

            INSERT INTO gescom.commander_articles
...
--
-- Doublure de structure pour la vue `v_details`
-- (Voir ci-dessous la vue réelle)
--
DROP VIEW IF EXISTS `v_details`;
CREATE TABLE IF NOT EXISTS `v_details` (
`Codes produits` varchar(8)
,`QteTot` decimal(32,0)
,`PrixTot` decimal(39,2)
);

-- --------------------------------------------------------

--
-- Doublure de structure pour la vue `v_ventes_zoom`
-- (Voir ci-dessous la vue réelle)
--
DROP VIEW IF EXISTS `v_ventes_zoom`;
CREATE TABLE IF NOT EXISTS `v_ventes_zoom` (
`pro_ref` varchar(8)
,`ode_id` int unsigned
,`ode_unit_price` decimal(7,2)
,`ode_discount` decimal(4,2)
,`ode_quantity` int
,`ode_ord_id` int unsigned
,`ode_pro_id` int unsigned
);

-- --------------------------------------------------------

--
-- Structure de la vue `v_details`
--
DROP TABLE IF EXISTS `v_details`;

DROP VIEW IF EXISTS `v_details`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_details`  AS  select `products`.`pro_ref` AS `Codes produits`,sum(`orders_details`.`ode_quantity`) AS `QteTot`,(`orders_details`.`ode_unit_price` * sum(`orders_details`.`ode_quantity`)) AS `PrixTot` from (`products` join `orders_details` on((`products`.`pro_id` = `orders_details`.`ode_pro_id`))) group by `products`.`pro_ref` order by `products`.`pro_ref` ;

-- --------------------------------------------------------

--
-- Structure de la vue `v_ventes_zoom`
--
DROP TABLE IF EXISTS `v_ventes_zoom`;

DROP VIEW IF EXISTS `v_ventes_zoom`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_ventes_zoom`  AS  select `products`.`pro_ref` AS `pro_ref`,`orders_details`.`ode_id` AS `ode_id`,`orders_details`.`ode_unit_price` AS `ode_unit_price`,`orders_details`.`ode_discount` AS `ode_discount`,`orders_details`.`ode_quantity` AS `ode_quantity`,`orders_details`.`ode_ord_id` AS `ode_ord_id`,`orders_details`.`ode_pro_id` AS `ode_pro_id` from (`orders_details` left join `products` on((`products`.`pro_id` = `orders_details`.`ode_pro_id`))) where (`products`.`pro_ref` = 'ZOOM') ;
